...
The Filter Processor allows filtering (dropping or keeping) spans, span events, metrics, datapoints, and logs from the collector.
...
An optional field `action` determines the behavior when conditions match.

| action | description                                                           |
|--------|-----------------------------------------------------------------------|
| drop   | Drop signals that match any conditions. This is the default behavior. |
| keep   | Retain signals that match any conditions and drop all others.         |
...
`error_mode`: allows overriding the top-level `error_mode` for this specific group of conditions. Valid values are `ignore`, `silent`, and `propagate`.

`action`: allows overriding the top-level `action` for this specific group of conditions. Valid values are `drop` and `keep`.
